...
class EmployeeController extends Controller
{
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        if ($request->has('image')) {
            $imageName = Helpers::upload('admin/', APPLICATION_IMAGE_FORMAT, $request->file('image'));
        } else {
            $imageName = 'def.png';
        }
        ...
        if (!empty($request->file('identity_image'))) {
            foreach ($request->identity_image as $img) {
                $identityImage = Helpers::upload('admin/', APPLICATION_IMAGE_FORMAT, $img);
    ...
    function list(Request $request): View|Factory|Application
    {
        ...
        $query = $this->admin->with(['role'])
            ->when($search != null, function ($query) use ($key, $search) {
        ...
        $sql = $query->toSql();
        $employees = $query->paginate($perPage)->appends($queryParam);

        return view('admin-views.employee.list', compact('employees','search', 'perPage'));
    }
