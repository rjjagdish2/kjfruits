...
;
        ...
        $discounts = $discounts->paginate($perPage)->appends($queryParam);

        $categories = $this->category->where(['parent_id'=>0])->orderBy('name')->get();

        return view('admin-views.discount.index', compact('discounts', 'categories','search', 'perPage'));
    }
    ...
    public function store(Request $request): RedirectResponse
    {
        $request->validate([
            ...
            'discount_amount' => $request->input('discount_type') === 'percent'
                ? 'required|numeric|min:0|max:100'
                : 'required|numeric|min:1',
            'maximum_amount' => 'required_if:discount_type,percent|numeric|min:1',
        ],[
            ...
            'discount_amount.min' => translate('Discount must be greater than or equal to 1'),
            'discount_amount.max' => translate('Discount percentage cannot exceed 100%'),
            'maximum_amount.required_if' => translate('Maximum amount is required when discount type is percent'),
            'maximum_amount.min' => translate('Maximum amount must be greater than or equal to 1'),
        ]);
