...
$total = $subtotal;
$sessionTotal = $subtotal - $discountOnProduct;
\Session::put('total', $sessionTotal);

$total -= $discountOnProduct;
...
if (session()->get('order_type') == 'home_delivery') {
    ...
    $productWeightCharge = Helpers::productWeightChargeCalculation(branchId: session()->get('branch_id') ?? 1, weight: $productWeight);
} else {
    $deliveryCharge = 0;
}
@endphp
...
<div class="box p-3">
    <dl class="row">
        ...
        <dd class="col-sm-6 text-right"> - {{ Helpers::set_symbol($discountOnProduct) }}</dd>
        ...
        <dd class="col-6 text-right text-info d-flex gap-2 justify-content-end">
            @if (count($cart ?? []) > 0)
                <button id="extra_discount_update_btn" class="btn btn-sm p-0 text-info" type="button"
            ...
            @if ($extraDiscountAmount > 0)
                <button id="extra_discount_delete_btn" class="btn btn-sm p-0 text-danger" type="button"
                    data-toggle="modal" data-target="#delete-discount">
                    <i class="tio-delete"></i>
                </button>
            @endif

            <span>- <span data-field="extraDiscountAmount">
        ...
        <dd class="col-sm-6 text-right">{{ Helpers::set_symbol($deliveryCharge) }}</dd>
        @if (session()->get('order_type') == 'home_delivery')
        ...
        @php($totalOrderAmount = $total + Helpers::convertStringAmountToNumber($updatedTotalTax) + $deliveryCharge + $productWeightCharge - $extraDiscountAmount)
        <dt class="col-sm-6">{{ translate('total') }} :</dt>
        <dd class="col-sm-6 text-right h4 b" data-field="totalOrderAmount">
            {{ Helpers::set_symbol($totalOrderAmount) }}
        </dd>
    </dl>
        ...
        <form action="{{ route('admin.pos.order') }}" id='order_place' method="post">
            ...
            <div class="collect-cash-section"
                ...
                <div class="form-group mb-2 d-flex align-items-center justify-content-between gap-2">
                    ...
                    <input type="number" class="form-control w-50 text-right" name="paid_amount" step="0.0000000001"
            ...
            <div class="modal-body">
                <form id="update_extra_discount_form">
                    ...
                    <div class="bg-light rounded p-3">
                        <label for="extra_discount_input">{{ translate('discount') }}</label>
                        ...
                        <small id="discount_error" class="d-flex justify-content-end fs-12px text-danger"></small>
                    </div>

                    <div class="btn--container justify-content-end mt-3">
                        <button class="btn btn-sm btn--reset" type="reset">{{ translate('reset') }}</button>
                        <button id="extra_discount_submit_btn" class="btn btn-sm btn--primary"
